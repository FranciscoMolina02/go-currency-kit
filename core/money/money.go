...
import (
	"errors"
	money "github.com/FranciscoMolina02/go-currency-kit"
	asset "github.com/FranciscoMolina02/go-currency-kit/core/currency"
	rounding "github.com/FranciscoMolina02/go-currency-kit/core/round"
	"math"
)
...
func NewMoney(amount float64, code string, roundingType rounding.RoundingType) *Money {
	roundedValue := round(amount, code, roundingType.String())
	return &Money{
		amount:   roundedValue,
		currency: asset.GetCurrency(code),
	}
}

func round(amount float64, code string, roundingType string) int64 {
	decimalAmount := asset.GetCurrency(code).DecimalAmount
	switch roundingType {
	case "ceiling":
		return int64(math.Ceil(amount * math.Pow10(decimalAmount)))
	case "floor":
		return int64(math.Floor(amount * math.Pow10(decimalAmount)))
	default:
		return int64(math.Round(amount * math.Pow10(decimalAmount)))
	}
}

// Currency returns the currency used by Money.
func (m *Money) Currency() *asset.Currency {
	return m.currency
}
...
func (m *Money) Absolute() *Money {
	return &Money{amount: money.mutate.calc.absolute(m.amount), currency: m.currency}
}

// Negative returns new Money struct from given Money using negative monetary value.
func (m *Money) Negative() *Money {
	return &Money{amount: money.mutate.calc.negative(m.amount), currency: m.currency}
}
...
func (m *Money) Add(om *Money) (*Money, error) {
	...
	return &Money{amount: money.mutate.calc.add(m.amount, om.amount), currency: m.currency}, nil
}
...
func (m *Money) Subtract(om *Money) (*Money, error) {
	...
	return &Money{amount: money.mutate.calc.subtract(m.amount, om.amount), currency: m.currency}, nil
}

// Multiply returns new Money struct with value representing Self multiplied value by multiplier.
func (m *Money) Multiply(mul int64) *Money {
	return &Money{amount: money.mutate.calc.multiply(m.amount, mul), currency: m.currency}
}
...
func (m *Money) Split(n int) ([]*Money, error) {
	...
	a := money.mutate.calc.divide(m.amount, int64(n))
	ms := make([]*Money, n)
	...
	r := money.mutate.calc.modulus(m.amount, int64(n))
	l := money.mutate.calc.absolute(r)
	...
	for p := 0; l != 0; p++ {
		ms[p].amount = money.mutate.calc.add(ms[p].amount, v)
		l--
	}
...
func (m *Money) Allocate(rs ...int) ([]*Money, error) {
	...
	for _, r := range rs {
		party := &Money{
			amount:   money.mutate.calc.allocate(m.amount, uint(r), sum),
			currency: m.currency,
		}
	...
	for p := 0; lo != 0; p++ {
		ms[p].amount = money.mutate.calc.add(ms[p].amount, sub)
		lo -= sub
	}
